...
import yuzData from '../data/games/yuzifadesi.json';
...
const YuzIfadesi = ({ player, onScore, onEndTurn }) => {
    ...
    useEffect(() => {
        let pool = JSON.parse(localStorage.getItem('game_pool_yuz_v2') || '[]');
        if (pool.length < 5) {
            pool = [...yuzData].sort(() => 0.5 - Math.random());
        }
        setPhrases(pool);
        localStorage.setItem('game_pool_yuz_v2', JSON.stringify(pool));
    }, []);
    ...
    const handleAction = (correct) => {
        ...
        if (nextPool.length === 0) {
            const fresh = [...yuzData].sort(() => 0.5 - Math.random());
            setPhrases(fresh);
            localStorage.setItem('game_pool_yuz_v2', JSON.stringify(fresh));
            setCurrentIndex(0);
        } else {
            setPhrases(nextPool);
            localStorage.setItem('game_pool_yuz_v2', JSON.stringify(nextPool));
    ...
    if (timeLeft === 0) {
        return (
            <div className="flex-center animate-slide-up" style={{ flexDirection: 'column', gap: '2rem', flex: 1, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '2rem' }}>Bilinen 0fade: <strong style={{ color: 'var(--success-color)' }}>{score}</strong></div>
    ...
    if (!isActive) {
        return (
            <div className="flex-center animate-slide-up" style={{ flexDirection: 'column', gap: '2rem', flex: 1, textAlign: 'center' }}>
                <h2 style={{ fontSize: '2.5rem' }}>Yz Ifadesi Tahmini</h2>
                <p style={{ color: 'var(--text-secondary)' }}>Ekranda yazan durumu SADECE YZ 0FADENLE gruba anlat! Vcut dili ve el hareketleri YASAK!</p>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: '1rem' }}>
            ...
            <div className="glass-panel animate-slide-up" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Mimiklerini Kullan:</span>
